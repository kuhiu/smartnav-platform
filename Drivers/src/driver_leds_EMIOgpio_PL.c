...
#define DEVICE_NAME "chardev_leds_EMIOgpio_PL"     /* Define Driver Name */
#define DEVICE_CLASS_NAME "class_leds_EMIOgpio_PL"
...
static int driver_open(struct inode *inode, struct file *file) 
{
    ...
    pr_info("Consegui la memoria \n");

    // Configure the portS as output
    set_registers(state.base_addr, OFFSET_GPIO_TRI, (0x03<<0), (0x00<<0) );
 ...
 static ssize_t driver_write(struct file *file, const char __user *ubuff, size_t size, loff_t *offset) 
{
    ...
    set_registers(state.base_addr, OFFSET_GPIO1_DATA, (0x03<<0), state.TX_buff[0] );
...
static ssize_t driver_read(struct file *file, char __user *ubuff, size_t size, loff_t *offset) 
{
    ...
    state.RX_buff[0] = ioread32(state.base_addr + OFFSET_GPIO1_DATA);
