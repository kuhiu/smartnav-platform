classdef navegador
    ...
    methods
        function nav = navegador()
            ...
            nav.obstacles = [ [8 5] [8 7] [8 9] [12 5] [12 7] [12 9] [12 11] [5 9] [5 11] [5 13] [5 15] [9 13] [9 15] [9 17] [9 19] [11 19] [13 19] [15 19] [11 24] [13 24] [15 24] [5 22] [5 24] [7 24] [9 24] [10 13] [12 13] [5 18] [5 20]];
            nav.robot = [10 4 pi/2];    % El tercer argumento es la orientacion absoluta del robot al iniciar 
            nav.target = [-1 -1];       % Esta simulacion no tiene target
        ...
        function nav = GoToTarget(nav,d)
            ...
            for i = 2:1000/nav.PeriodoDeSampleo
                ...
                if (distL_O <= 0.5 || distC_O <= 0.5 || distR_O <= 0.5)
                    nav.NOseguro = true;
                    display("Colisiono")
                    break
                end
