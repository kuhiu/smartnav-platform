...
int vdma_start(void){
  ...
  // Reset VDMA
  set_registers(vdma_handle.base_addr_ctrl, OFFSET_VDMA_S2MM_CONTROL_REGISTER, VDMA_CONTROL_REGISTER_START, 0);
  ...
  while ( ( (ioread32(vdma_handle.base_addr_ctrl+OFFSET_VDMA_S2MM_CONTROL_REGISTER) >> 2 ) & 0x01) != 0 ) {
    pr_info(DEVICE_NAME": Esperando el reset...\n");
    mdelay(1);
  }
  pr_info(DEVICE_NAME": Reset completo \n");
