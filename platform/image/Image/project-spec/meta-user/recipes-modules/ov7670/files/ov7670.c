 ...
 *              1 = 30fps
 */
	{ REG_CLKRC, 0x40 },	/* OV: external clock */
	{ REG_TSLB,  0x04 },	/* OV */
	{ REG_COM7,  0x11 },	/* QVGA */
	/*
			...
			const struct i2c_device_id *id)
{
	...
	printk(KERN_ERR "Entre a ov7670 probe\n");
	...
	printk(KERN_ERR "Llegue 3\n");
	info->clk = devm_clk_get(&client->dev, "xclk");
	if (IS_ERR(info->clk))
		return PTR_ERR(info->clk);
	printk(KERN_ERR "Llegue 3.2\n");
	ret = clk_prepare_enable(info->clk);
	if (ret)
		return ret;
	printk(KERN_ERR "Llegue 4\n");
	info->clock_speed = clk_get_rate(info->clk) / 1000000;
	if (info->clock_speed < 10 || info->clock_speed > 48) {
		ret = -EINVAL;
		goto clk_disable;
	}
	printk(KERN_ERR "Llegue 4.1\n");
	ret = ov7670_init_gpio(client, info);
	if (ret)
		goto clk_disable;
	printk(KERN_ERR "Llegue 4.2\n");
	ov7670_s_power(sd, 1);
	printk(KERN_ERR "Llegue 5\n");
