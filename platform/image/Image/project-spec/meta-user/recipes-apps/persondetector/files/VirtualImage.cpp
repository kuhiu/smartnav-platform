...
VirtualImage::VirtualImage(uint32_t width, uint32_t height, uint32_t channels, colorSpace color_space, void *data) 
: __width(width), __height(height), __channels(channels), __color_space(color_space) {
	switch (channels) {
	case 1:
		__data.create(height, width, CV_8UC1);
		memcpy(__data.data, data, height*width*1);
		__type = CV_8UC1;
		break;
	case 3:
		__data.create(height, width, CV_8UC3);
		memcpy(__data.data, data, height*width*3);
		__type = CV_8UC3;
		break;
	case 4:
...
void VirtualImage::rgbaToRgb() {
	if ( __color_space == colorSpace(RGBA)) {
		printf("Convert RGBA to RGB \n");
		cv::cvtColor(__data, __data, cv::COLOR_RGBA2RGB);
		__data.convertTo(__data, CV_8UC3);
		__color_space = colorSpace(RGB);
		__type = CV_8UC3;
	}
	else 
...
colorSpace VirtualImage::getColorSpace() const { return __color_space; };
