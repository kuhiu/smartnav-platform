...
public:
   ...
   * @param image 
   */
  std::vector<RecognitionResult> detect(const VirtualImage &image);
...
private:
  ...
  TfLiteStatus __fillInput(uint32_t image_width, uint32_t image_height, uint32_t channels, const VirtualImage &image);
  /** Neural network post processing */
  std::vector<RecognitionResult> __postProcess(const VirtualImage &image);
